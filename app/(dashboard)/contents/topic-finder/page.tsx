...
export default function TopicFinderPage() {
    ...
    const handleGenerateDetail = async () => {
        if (!selectedHeadline) {
            toast.error(t('pages.contents.topicFinder.writeOrAddTopic'));
            return;
        }
        ...
        try {
            ...
            toast.success(t('pages.contents.topicFinder.threadsGenerated', { count: threadChain.length }));
        } catch (e) {
            toast.error(t('pages.contents.topicFinder.failedToGenerateChain'));
    ...
    useEffect(() => {
        ...
        const fetchAccountDetails = async () => {
            ...
            try {
                ...
                toast.error(t('pages.contents.topicFinder.accountInfoLoadError'))
            } finally {
                setIsLoading(false)
            }
        }
    ...
    const generateTopics = async () => {
        if (!accountInfo) {
            toast.error(t('pages.contents.topicFinder.noAccountInfo'));
            return;
        }
        ...
        try {
            ...
            if (!Array.isArray(newTopics)) {
                toast.error(t('pages.contents.topicFinder.invalidTopicFormat'));
                return;
            }
            ...
            toast.error(t('pages.contents.topicFinder.failedToGenerateTopics'));
        } finally {
    ...
    return (
        <div className="p-4 md:p-6 h-screen">
            <div className="flex flex-col items-center justify-center h-full">
                <div className="w-full mx-auto pb-32 flex flex-col items-center overflow-y-auto [&::-webkit-scrollbar]:hidden [-ms-overflow-style:'none'] [scrollbar-width:'none']">
                    {/* 중앙 정렬 인사말 */}
                    <div className="">
                        <div className="flex flex-row items-center gap-2 mb-1">
                            ...
                            {locale === 'en' && (
                                <h2 className="text-2xl font-semibold text-left">
                                    Hello, {mounted ? (currentUsername || 'User') : 'User'},
                                </h2>
                            )}
                            {locale === 'ko' && (
                                <h2 className="text-2xl font-semibold text-left">
                                    안녕하세요, {mounted ? (currentUsername || 'User') : 'User'}님!
                                </h2>
                            )}
                        </div>
                        <h2 className="text-2xl font-semibold text-left">{t('pages.contents.topicFinder.question')}</h2>
                    </div>
