...
// dateRanges will be defined inside component to access t()
...
export default function StatisticsPage() {
    ...
    const dateRanges: DateRange[] = [
        ...
        { label: t('components.statistics.dateRanges.30days'), days: 30 },
        { label: t('components.statistics.dateRanges.90days'), days: 90 },
    ];
    ...
    const metricCards = [
        {
            title: t('components.statistics.metrics.totalViews'),
            value: getInsightValue('views') || t('components.statistics.metrics.noData'),
            change: changes?.views?.change || 'N/A',
            changeType: changes?.views?.changeType || 'neutral' as const,
            icon: <Eye className="w-5 h-5" />,
            description: t('components.statistics.metrics.trendInPeriod')
        },
        {
            title: t('components.statistics.metrics.totalFollowers'),
            value: getInsightValue('followers_count') || t('components.statistics.metrics.noData'),
            icon: <Users className="w-5 h-5" />,
            description: t('components.statistics.metrics.currentCount')
        },
        {
            title: t('components.statistics.metrics.totalLikes'),
            value: getInsightValue('likes') || t('components.statistics.metrics.noData'),
            change: changes?.likes?.change || '',
            changeType: changes?.likes?.changeType || 'neutral' as const,
            icon: <Heart className="w-5 h-5" />,
            description: t('components.statistics.metrics.fromPreviousPeriod')
        },
        {
            title: t('components.statistics.metrics.totalReplies'),
            value: getInsightValue('replies') || t('components.statistics.metrics.noData'),
        ...
        {
            title: t('components.statistics.metrics.totalReposts'),
            value: getInsightValue('reposts') || t('components.statistics.metrics.noData'),
        ...
        {
            title: t('components.statistics.metrics.totalQuotes'),
            value: getInsightValue('quotes') || t('components.statistics.metrics.noData'),
        ...
        return (
            <div className="space-y-6 p-4 md:p-6">
                <div className="flex flex-col items-center justify-center py-12 text-center">
                    ...
                    <h2 className="text-xl font-semibold mb-2">{t('components.statistics.noAccount.title')}</h2>
                    <p className="text-muted-foreground mb-4">
                        {t('components.statistics.noAccount.description')}
                    </p>
                    <Button onClick={() => window.location.href = "/api/threads/oauth"}>
                        {t('components.statistics.noAccount.connectButton')}
                    </Button>
                </div>
            </div>
        );
        ...
        return (
            <div className="container mx-auto py-6 space-y-6">
                <div className="text-center">
                    <h1 className="text-2xl font-bold mb-4">{t('components.statistics.noAuth.title')}</h1>
                    <p className="text-muted-foreground">{t('components.statistics.noAuth.description')}</p>
                </div>
            </div>
        );
    }
        ...
        <div className="h-full overflow-y-auto space-y-4 pb-8 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:'none'] [scrollbar-width:'none']">
            ...
            <div className="sticky top-0 z-10 bg-white p-2 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                <div>
                    <div className="flex items-center gap-3">
                        <h1 className="text-2xl md:text-3xl font-bold">{t('components.statistics.title')}</h1>
                ...
                <div className="flex gap-3">
                    ...
                    <div className="flex border rounded-lg bg-muted p-1 w-fit sm:w-auto">
                        ...
                        >
                            ...
                            {t('components.statistics.refresh')}
                        </button>
                    </div>
                </div>
            </div>
            ...
            {isLoading && !isFromCache ? (
                ...
                <>
                    ...
                    <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
                        ...
                        <Card>
                            <CardHeader className="pb-4">
                                <CardTitle className="text-lg">{t('components.statistics.charts.viewsPerformance')}</CardTitle>
                                <CardDescription className="text-sm text-muted-foreground">
                                    {t('components.statistics.charts.forLastDays', { days: selectedDateRange })}
                                </CardDescription>
                            </CardHeader>
                            <CardContent>
                        ...
                        <Card>
                            <CardHeader>
                                <CardTitle className="text-lg">{t('components.statistics.charts.engagementBreakdown')}</CardTitle>
                                <CardDescription className="text-sm text-muted-foreground">
                                    {t('components.statistics.charts.forLastDays', { days: selectedDateRange })}
                                </CardDescription>
                            </CardHeader>
                    ...
                    <Card>
                        <CardHeader>
                            <CardTitle className="text-lg">{t('components.statistics.topPosts.title')}</CardTitle>
                            <p className="text-sm text-muted-foreground">
                                {t('components.statistics.topPosts.description', { days: selectedDateRange })}
                            </p>
                        </CardHeader>
                        <CardContent>
                            {topPosts.length > 0 ? (
                                ...
                                <div className="text-center py-8">
                                    <p className="text-muted-foreground">
                                        {t('components.statistics.topPosts.noData')}
                                    </p>
                                </div>
                            )}
                        </CardContent>
