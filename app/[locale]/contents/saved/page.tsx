...
export default function SavedPage() {
    ...
    const handleAddUrl = () => {
        if (!isValidThreadsUrl(url)) {
            setError(t('pages.contents.saved.invalidUrl'));
            return;
        }

        if (!currentSocialId) {
            setError(t('pages.contents.saved.selectSocialAccount'));
            return;
        }
        ...
        startTransition(async () => {
            try {
                ...
                toast.success(t('pages.contents.saved.savedSuccessfully'));
            } catch (err) {
                console.error(err);
                toast.error(t('pages.contents.saved.saveError'));
            }
        });
    };
    ...
    return (
        <div className="h-full w-full overflow-hidden flex flex-col p-6">
            <h1 className="text-3xl font-bold text-zinc-700 mb-6">{t('pages.contents.saved.title')}</h1>
                ...
                <div className="flex-1 flex flex-col overflow-hidden">
                    <div className="flex flex-col gap-4 mb-6 bg-white p-4 rounded-[20px]">
                        <div className="flex gap-2 items-center">
                            <div className="text-lg font-bold">{t('pages.contents.saved.addByUrl')}</div>
                            <p className="text-sm text-muted-foreground">
                                {t('pages.contents.saved.addByUrlDescription')}
                            </p>
                        </div>
                        <div className="flex gap-2">
                            <Input
                                id="url"
                                placeholder={t('pages.contents.saved.urlPlaceholder')}
                                value={url}
                                onChange={(e) => setUrl(e.target.value)}
                            />
                            <Button onClick={handleAddUrl} disabled={isPending}>
                                {isPending ? t('pages.contents.saved.saving') : t('pages.contents.saved.addByUrlButton')}
                            </Button>
                        </div>
